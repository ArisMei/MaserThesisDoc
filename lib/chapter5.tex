\section{CAPÍTULO 5: RESULTADOS}\label{cap:results}
...
\subsubsection{Frecuencia Cardíaca y Saturación de Oxígeno Considerando Variabilidad entre Pacientes Diferente}

Tanto para \textit{Frecuencia Cardíaca} y \textit{Saturación de Oxígeno} se ha realizado el mismo estudio pero considerando que la variabilidad entre pacientes es diferente, simplemente se muestran los gráficos cuando se considera en la ejecución \texttt{var.equal   = FALSE} (\textit{p-valor} escalada no variará ya que la variabilidad en el conjunto total de los datos es la misma).

\begin{figure}[H]
    \centering
    \includegraphics[scale = 0.75]{./img/mean-FCvar.png}
    \caption{p-valor de la media horaria de la \textit{Frecuencia Cardíaca} entre pacientes que sufren DETERIORO y los que no considerando \texttt{var.equal   = FALSE}}
    \label{fig:mean-FC-var}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale = 0.75]{./img/mean-SatO2var.png}
    \caption{p-valor de la media horaria de la \textit{Saturación de Oxígeno} entre pacientes que sufren DETERIORO y los que no considerando \texttt{var.equal   = FALSE}}
    \label{fig:mean-SatO2-var}
\end{figure}
...
\begin{enumerate}
    ...
    \item Clasificación Discriminate mediante Random Forest de los clústeres en función de los datos utilizados para generar los mismos clústeres (Raw Data, FAS, Periodograma y FCC). Se mostrará la importancia para identificar qué datos contribuyen más en la clasificación entre k = 2 clusters generados.
    ...
    \item Cálculo de las medias de todos los valores empleados (Raw Data, FAS, Periodograma y FCC) entre todos los pacientes, y diagrama de sus valores entre k = 2 clusters generados.
\end{enumerate}
...
\newpage
\include{lib/chapter5-Raw-data.tex}

\newpage
\include{lib/chapter5-FAS.tex}

\newpage
\include{lib/chapter5-Peridiograma.tex}

\newpage
\include{lib/chapter5-FCC.tex}

\newpage
\include{lib/chapter5-Adj-Rand-Index.tex}

\newpage
\include{lib/chapter5-FINAL-MODEL.tex}
