...
La imputación de datos se realiza según el código que se muestra en el Anexo~\ref{lst:input-gower-fun}. El código funciona de la siguiente forma: 
\begin{enumerate}
    \item Antes de nada es necesario seleccionar el conjunto de datos que van a ser utilizados para la imputación (no deben tener datos faltantes), el paciente al que se le quieren inputar datos y en último lugar la cantidad $k$ de \textit{vecinos} que se van a tener en cuenta. 
    \item En segundo lugar se calcula la distancia de Gower entre el paciente seleccionado y el resto de pacientes teniendo solo en cuenta las variables sin datos faltantes. La distancia solo se calcula entre los pacientes que tienen datos en las variables que se quieren imputar. 
    \item Se imputan los datos faltantes con la media de los $k$ pacientes más parecidos.
    \item 
...
Se calcula la distancia de Gower entrcogen lo 7 pacientes más parecidos y se imputan las medias de sus valores en el caso de las variable 
